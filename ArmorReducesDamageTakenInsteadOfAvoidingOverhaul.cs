// Project:         ArmorReducesDamageTakenInsteadOfAvoidingOverhaul mod for Daggerfall Unity (http://www.dfworkshop.net)
// Copyright:       Copyright (C) 2020 Kirk.O
// License:         MIT License (http://www.opensource.org/licenses/mit-license.php)
// Author:          Kirk.O
// Created On: 	    3/17/2020, 6:30 PM
// Last Edit:		3/30/2020, 2:00 AM
...
namespace ArmorReducesDamageTakenInsteadOfAvoidingOverhaul
{
    public class ArmorReducesDamageTakenInsteadOfAvoidingOverhaul : MonoBehaviour
    {
        static Mod mod;
        ...
        {
            mod = initParams.Mod;
            var go = new GameObject("ArmorReducesDamageTakenInsteadOfAvoidingOverhaul");
            go.AddComponent<ArmorReducesDamageTakenInsteadOfAvoidingOverhaul>();
        }
        ...
        {
            Debug.Log("Begin mod init: ArmorReducesDamageTakenInsteadOfAvoidingOverhaul");
            ...
            if (equipmentDamageEnhanced)
            {
				...
				Debug.Log("ArmorReducesDamageTakenInsteadOfAvoidingOverhaul: Enhanced Equipment Damage Module Active");
            }
			else
				Debug.Log("ArmorReducesDamageTakenInsteadOfAvoidingOverhaul: Enhanced Equipment Damage Module Disabled");
			
			if (fixedStrengthDamageModifier)
			{
				FormulaHelper.RegisterOverride(mod, "DamageModifier", (Func<int, int>)DamageModifier);
				
				Debug.Log("ArmorReducesDamageTakenInsteadOfAvoidingOverhaul: Fixed Strength Damage Modifier Module Active");
			}
			else
				Debug.Log("ArmorReducesDamageTakenInsteadOfAvoidingOverhaul: Fixed Strength Damage Modifier Module Disabled");
			...
			if (armorHitFormulaRedone)
			{
				...
				FormulaHelper.RegisterOverride(mod, "CalculateSuccessfulHit", (Func<DaggerfallEntity, DaggerfallEntity, int, int, int>)CalculateSuccessfulHit);
				...
				Debug.Log("ArmorReducesDamageTakenInsteadOfAvoidingOverhaul: Armor Hit Formula Redone Module Active");
			}
			else
				Debug.Log("ArmorReducesDamageTakenInsteadOfAvoidingOverhaul: Armor Hit Formula Redone Module Disabled");
			
			if (criticalStrikesIncreaseDamage)
			{
				if (armorHitFormulaRedone)
				{
					Debug.Log("ArmorReducesDamageTakenInsteadOfAvoidingOverhaul: Critical Strikes Increase Damage Module Active");
				...
				else
				{
					...
					Debug.Log("ArmorReducesDamageTakenInsteadOfAvoidingOverhaul: Critical Strikes Increase Damage Module Is Dependent On Armor Hit Formula Redone To Function, So By Default is Disabled");
				}
			}
			else
			{
				critStrikeModuleCheck = false;
				Debug.Log("ArmorReducesDamageTakenInsteadOfAvoidingOverhaul: Critical Strikes Increase Damage Module Disabled");
			}
			...
			if (rolePlayRealismArcheryModule)
			{
				...
				Debug.Log("ArmorReducesDamageTakenInsteadOfAvoidingOverhaul: Roleplay Realism's Archery Module Active");
			...
			else
			{
				...
				Debug.Log("ArmorReducesDamageTakenInsteadOfAvoidingOverhaul: Roleplay Realism's Archery Module Disabled");
			}
			
			if (ralzarMeanerMonstersEdit)
			{
				Debug.Log("ArmorReducesDamageTakenInsteadOfAvoidingOverhaul: Ralzar's Meaner Monsters Edited Module Active");
				Debug.Log("ArmorReducesDamageTakenInsteadOfAvoidingOverhaul: Initializing Enemy Stat Values, Ralzar's Meaner Monsters Edited Module...");
				...
				Debug.Log("ArmorReducesDamageTakenInsteadOfAvoidingOverhaul: initialization Complete For Enemy Stat Values, Ralzar's Meaner Monsters Edited Module");
			}
			else
				Debug.Log("ArmorReducesDamageTakenInsteadOfAvoidingOverhaul: Ralzar's Meaner Monsters Edited Module Disabled");

            Debug.Log("Finished mod init: ArmorReducesDamageTakenInsteadOfAvoidingOverhaul");
        }
		...
		private static int CalculateAttackDamage(DaggerfallEntity attacker, DaggerfallEntity target, int enemyAnimStateRecord, int weaponAnimTime, DaggerfallUnityItem weapon)
		{
            ...
            if (attacker == player)
            {
                ...
                if (weapon != null)
                {
                    ...
                    {
                        damageModifiers += ((attacker.Level / 2) + 1); // Buffed from /3 to /2
                ...
                if (weapon != null)
                {
                    ...
                    {
                        damageModifiers += (attacker.Level / 2); // Buffed Racial Mod from /4 to /2
                        chanceToHitMod += (attacker.Level / 2); // Buffed Racial Mod from /4 to /2
                    }
                    else if (skillID == (short)DFCareer.Skills.Archery)
                    {
                        if (player.RaceTemplate.ID == (int)Races.WoodElf)
                        {
                            ...
                            chanceToHitMod += (attacker.Level / 1); // Buffed Racial Mod from /3 to /1
                        }
                    }
            ...
            if (skillID == (short)DFCareer.Skills.HandToHand)
            {
                ...
                if (attacker == player || (AIAttacker != null && AIAttacker.EntityType == EntityTypes.EnemyClass))
                {
                    if (CalculateSuccessfulHit(attacker, target, chanceToHitMod, struckBodyPart) > 0)
                    {
                        ...
                        if (attacker == player)
                            damage += (DamageModifier(attacker.Stats.LiveStrength)) * 2; // Multiplying by 2, so that Hand-to-hand gets double the damage mod from Strength.
                        // Handle backstabbing
            ...
            else if (weapon != null)
            {
				...
				chanceToHitMod += ((weapon.GetWeaponMaterialModifier() * 2) + 2);
		...
		private static int CalculateSuccessfulHit(DaggerfallEntity attacker, DaggerfallEntity target, int chanceToHitMod, int struckBodyPart)
		{
			if (attacker == null || target == null)
                return 0;
			...
			//Debug.LogFormat("After Luck = {0}", chanceToHit);

            // Apply agility modifier.
            chanceToHit += ((attacker.Stats.LiveAgility - target.Stats.LiveAgility) / 4); //Made Agility have twice as much effect on final hit chance.
			//Debug.LogFormat("After Agility = {0}", chanceToHit);
			
			// Possibly make the Speed Stat a small factor as well, seems like it would make sense.
			chanceToHit += ((attacker.Stats.LiveSpeed - target.Stats.LiveSpeed) / 8);
			//Debug.LogFormat("After Speed = {0}", chanceToHit);
			...
			chanceToHit -= (int)Mathf.Round((float)(target.Stats.LiveLuck - 50) / 10); // With this, at most Luck will effect chances by either -5 or +5.
			
            // Apply dodging modifier.
            chanceToHit -= (target.Skills.GetLiveSkillValue(DFCareer.Skills.Dodging) / 2); // Changing 4 to a 2, so 100 dodge will give -50 to hit chance, very powerful.
			//Debug.LogFormat("After Dodge = {0}", chanceToHit);
			...
			if (!critStrikeModuleCheck) // Applies the mostly unmodified version of critical strike if the module in settings for the redone version is disabled.
			{
				...
				if (attacker == player)
				{
					...
					{
						chanceToHit += (attacker.Skills.GetLiveSkillValue(DFCareer.Skills.CriticalStrike) / 3); // Now for player, at 100 crit skill, "crit" success will give 33 more hit-mod
					...
					//Debug.LogFormat("After Crit, Player = {0}", chanceToHit); // May consider eventually moving/adding the Critical Strike equation to the damage modifier formula, likely not in 1.0 of this mod, but maybe later on.
				}
				...
				else
				{
					...
					{
						chanceToHit += (attacker.Skills.GetLiveSkillValue(DFCareer.Skills.CriticalStrike) / 10); // For monsters, works like normal. "crit" success will give 10 more hit-mod
					}
					//Debug.LogFormat("After Crit, Monster = {0}", chanceToHit);
				}
			}
            ...
            if ((target != player) && (AITarget.EntityType == EntityTypes.EnemyMonster))
            {
                chanceToHit += 50; // Changed from 40 to 50, +10, in since i'm going to make dodging have double the effect, as well as nerf weapon material hit mod more.
            }

            // DF Chronicles says -60 is applied at the end, but it actually seems to be -50.
            chanceToHit -= 50;
			//Debug.LogFormat("Final chanceToHit Mod = {0}", chanceToHit);
		...
		private static int CalculateArmorDamageReductionWithWeapon(DaggerfallEntity attacker, DaggerfallEntity target, int damage, DaggerfallUnityItem weapon, int struckBodyPart, float naturalDamResist)
		{
			...
			if (shieldTakesDamage)
			{
				...
				damage = PercentageReductionCalculationWithWeapon(shield, armorMaterial, atkStrength, damage, bluntWep, wepWeight, shieldTakesDamage, naturalDamResist);
			}
			else
			{
				EquipSlots hitSlot = DaggerfallUnityItem.GetEquipSlotForBodyPart((BodyParts)struckBodyPart);
				...
				if (armor != null)
				{
					...
					damage = PercentageReductionCalculationWithWeapon(armor, armorMaterial, atkStrength, damage, bluntWep, wepWeight, shieldTakesDamage, naturalDamResist);
				}
		...
		private static int CalculateArmorDamageReductionWithUnarmed(DaggerfallEntity attacker, DaggerfallEntity target, int damage, int struckBodyPart, float naturalDamResist)
		{
			...
			if (shieldTakesDamage)
			{
				...
				damage = PercentageReductionCalculationWithUnarmed(shield, armorMaterial, atkStrength, damage, shieldTakesDamage, naturalDamResist);
			}
			else
			{
				EquipSlots hitSlot = DaggerfallUnityItem.GetEquipSlotForBodyPart((BodyParts)struckBodyPart);
				...
				if (armor != null)
				{
					...
					damage = PercentageReductionCalculationWithUnarmed(armor, armorMaterial, atkStrength, damage, shieldTakesDamage, naturalDamResist);
				}
				else // If the body part struck in 'naked' IE has no armor or shield protecting it.
				{
					damage = (int)Mathf.Round(damage*(1f - naturalDamResist));
